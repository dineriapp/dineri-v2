...
import { hostnameOf, isVenueMode, venueSiteUrl, venueUrl } from "@/lib/venue-url";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const onVenueHost = isVenueMode() && host === hostnameOf(venueSiteUrl());
  const wantMarketing = !onVenueHost;
  const wantVenues = !isVenueMode() || onVenueHost;

  const deployedAt = new Date();

  const marketing: MetadataRoute.Sitemap = wantMarketing
    ? MARKETING_PATHS.map(({ path, priority, changeFrequency }) => ({
  ...
  const venues = wantVenues ? await listVenues() : [];
  ...
  const venuePages: MetadataRoute.Sitemap = venues.flatMap((venue) => {
    ...
    if (getOnlineReservationsStatus(venue.reservation_settings).available) {
      entries.push({
        url: venueUrl(venue.slug, "/reserve"),
        lastModified: venue.updatedAt,
        changeFrequency: "weekly",
        priority: 0.7,
      });
